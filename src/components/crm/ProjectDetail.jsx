...
import {
  ...
  PrimaryBtn, GhostBtn, SolidChipBtn, Card, DashedAdd, SkeletonList, hair, dueLabel, fmtShort, fmtRel, STATUS_ORDER, STATUS_LABEL,
} from './ui.jsx';
...
export default function ProjectDetail({ projectId, profile, onClose, onSelectTask, onNavigate }) {
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--scene)' }}>
      {/* ── Header ── */}
      <div className="px-6 pt-5 pb-[18px] border-b" style={hair}>
        <div className="flex items-start gap-4">
          <div className="flex-1 min-w-0">
            ...
            <PageTitle size={28} className="mb-2 truncate">{project.name}</PageTitle>
      ...
      <div className="flex-1 overflow-y-auto">
        {editing ? (
          ...
          <div className="px-6 pt-[18px] pb-6 grid gap-[18px] lg:grid-cols-[minmax(0,1fr)_300px]">
            {/* LEFT — phases */}
                    ...
                    {g.tasks.map((t, i) => {
                      ...
                      return (
                        <div key={t.id} className={`flex items-center gap-3 px-4 py-[11px] ${i < g.tasks.length - 1 ? 'border-b' : ''}`}
                          ...
                          {!isDone && <Mono tone={due.tone === 'coral' ? 'coral' : due.tone === 'primary' ? 'primary' : 'dim'} bold={due.tone !== 'dim'}>{due.text}</Mono>}
