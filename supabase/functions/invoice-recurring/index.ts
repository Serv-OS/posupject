...
import { invoiceEmailHtml, sendInvoiceEmail, moneyFor, taxLabelFor, dateLocaleFor, balanceDue, amountAllocated } from "../_shared/invoiceEmail.ts";
  ...
  try {
    ...
    try {
      ...
      for (const inv of (pending || [])) {
        ...
        // Never ask for money a credit note, or credit applied from another
        // invoice's credit note, has already cancelled out. The database only
        // credits sent, viewed or paid invoices and only applies credit to
        // sent or viewed ones, so a draft here should never carry either; this
        // keeps it that way if one ever does. balanceDue takes both off. A
        // zero-total draft with no credit still goes out as it always has.
        const anyCredit = Number((inv as any).amount_credited || 0) > 0 || amountAllocated(inv) > 0;
        if (anyCredit && balanceDue(inv) <= 0) continue;
